/*
 * Box 77 - Service Facility production PREVIEW.
 *
 * Edit the four literals in manual_inputs, then run this statement in Toad.
 * To request a plan, replace CAST(NULL AS VARCHAR2(36)) with 'the-plan-guid'.
 * Valid mode/address pairs are ALWAYS/Y, ALWAYS/N, CONDITIONAL/Y,
 * CONDITIONAL/N, and NEVER/N. The statement installs nothing and changes no
 * data. Invalid input or ambiguous production state returns BLOCKED rows.
 */
WITH manual_inputs AS (
    SELECT
        'PUT_PAYOR_GUID_HERE' AS payor_guid,
        CAST(NULL AS VARCHAR2(36)) AS plan_guid,
        'CONDITIONAL' AS service_facility_mode,
        'N' AS report_address
    FROM dual
),
params AS (
    SELECT
        ...
        UPPER(TRIM(service_facility_mode)) AS service_facility_mode,
        UPPER(TRIM(report_address)) AS report_address,
        CASE
            WHEN TRIM(payor_guid) IS NULL
              OR TRIM(payor_guid) = 'PUT_PAYOR_GUID_HERE'
                THEN 'PAYOR_GUID_REQUIRED'
            WHEN UPPER(TRIM(service_facility_mode)) NOT IN (
                'ALWAYS', 'CONDITIONAL', 'NEVER'
            ) THEN 'INVALID_SERVICE_FACILITY_MODE'
            WHEN UPPER(TRIM(report_address)) NOT IN ('Y', 'N')
                THEN 'INVALID_REPORT_ADDRESS'
            WHEN UPPER(TRIM(service_facility_mode)) = 'NEVER'
             AND UPPER(TRIM(report_address)) <> 'N'
                THEN 'NEVER_REQUIRES_ADDRESS_N'
            ELSE 'VALID'
        END AS input_status,
        CASE
            WHEN UPPER(TRIM(service_facility_mode)) = 'ALWAYS'
             AND UPPER(TRIM(report_address)) = 'Y'
                THEN 'SERVICE_FACILITY_ALWAYS_ADDRESS_YES'
            WHEN UPPER(TRIM(service_facility_mode)) = 'ALWAYS'
             AND UPPER(TRIM(report_address)) = 'N'
                THEN 'SERVICE_FACILITY_ALWAYS_ADDRESS_NO'
            WHEN UPPER(TRIM(service_facility_mode)) = 'CONDITIONAL'
             AND UPPER(TRIM(report_address)) = 'Y'
                THEN 'SERVICE_FACILITY_CONDITIONAL_ADDRESS_YES'
            WHEN UPPER(TRIM(service_facility_mode)) = 'CONDITIONAL'
             AND UPPER(TRIM(report_address)) = 'N'
                THEN 'SERVICE_FACILITY_CONDITIONAL_ADDRESS_NO'
            WHEN UPPER(TRIM(service_facility_mode)) = 'NEVER'
             AND UPPER(TRIM(report_address)) = 'N'
                THEN 'SERVICE_FACILITY_NEVER'
        END AS option_code
    FROM manual_inputs
),
targets AS (
    SELECT
        1 AS target_order,
        'NM1' AS target_segment,
        'D2310E2500NM1343' AS record_type_code
    FROM dual
    UNION ALL
    SELECT 2, 'N3', 'D2310E2650N3346'
    FROM dual
    UNION ALL
    SELECT 3, 'N4', 'D2310E2700N4347'
    FROM dual
),
requested_targets AS (
    SELECT
        ...
        p.service_facility_mode,
        p.report_address,
        p.input_status,
        p.option_code,
        CASE
            WHEN t.target_segment = 'NM1'
             AND p.service_facility_mode = 'ALWAYS' THEN 'RETURN_1'
            WHEN t.target_segment = 'NM1'
             AND p.service_facility_mode = 'CONDITIONAL'
                THEN 'G_D2310E2500NM1343_COUNT'
            WHEN t.target_segment = 'NM1'
             AND p.service_facility_mode = 'NEVER' THEN 'RETURN_0'
            WHEN t.target_segment IN ('N3', 'N4')
             AND p.service_facility_mode = 'ALWAYS'
             AND p.report_address = 'Y' THEN 'RETURN_1'
            WHEN t.target_segment IN ('N3', 'N4')
             AND p.service_facility_mode = 'CONDITIONAL'
             AND p.report_address = 'Y'
                THEN 'G_D2310E2500NM1343_COUNT'
            WHEN t.target_segment IN ('N3', 'N4') THEN 'RETURN_0'
        END AS desired_her_sto_proc_name,
        CASE
            WHEN t.target_segment = 'NM1'
             AND p.service_facility_mode IN ('ALWAYS', 'CONDITIONAL') THEN 'Y'
            WHEN t.target_segment IN ('N3', 'N4')
             AND p.report_address = 'Y'
             AND p.service_facility_mode IN ('ALWAYS', 'CONDITIONAL') THEN 'Y'
            ELSE 'N'
...
requested_overlays AS (
    SELECT 'NM1' AS target_segment, '01' AS field_number,
           CAST(NULL AS VARCHAR2(10)) AS sto_action,
           CAST(NULL AS VARCHAR2(30)) AS desired_sto_proc_name,
           'SET' AS hard_action,
           '77' AS desired_hard_coded_data
    FROM params p
    WHERE p.service_facility_mode IN ('ALWAYS', 'CONDITIONAL')
    UNION ALL
    SELECT 'NM1', '02', NULL, NULL, 'SET', '2'
    FROM params p
    WHERE p.service_facility_mode IN ('ALWAYS', 'CONDITIONAL')
    UNION ALL
    SELECT 'NM1', '03', 'SET', 'G_ORGANIZATION_NAME', NULL, NULL
    FROM params p
    WHERE p.service_facility_mode IN ('ALWAYS', 'CONDITIONAL')
    UNION ALL
    SELECT 'NM1', '09', 'SET', 'G_FACILITY_NPI', NULL, NULL
    FROM params p
    WHERE p.service_facility_mode IN ('ALWAYS', 'CONDITIONAL')
    UNION ALL
    SELECT 'N3', '01', 'SET', 'G_CARE_LOCATION_ADDR1', NULL, NULL
    FROM params p
    WHERE p.report_address = 'Y'
      AND p.service_facility_mode IN ('ALWAYS', 'CONDITIONAL')
    UNION ALL
    SELECT 'N3', '02', 'SET', 'G_CARE_LOCATION_ADDR2', NULL, NULL
    FROM params p
    WHERE p.report_address = 'Y'
      AND p.service_facility_mode IN ('ALWAYS', 'CONDITIONAL')
    UNION ALL
    SELECT 'N4', '01', 'SET', 'G_CARE_LOCATION_CITY', NULL, NULL
    FROM params p
    WHERE p.report_address = 'Y'
      AND p.service_facility_mode IN ('ALWAYS', 'CONDITIONAL')
    UNION ALL
    SELECT 'N4', '02', 'SET', 'G_CARE_LOCATION_STATE', NULL, NULL
    FROM params p
    WHERE p.report_address = 'Y'
      AND p.service_facility_mode IN ('ALWAYS', 'CONDITIONAL')
    UNION ALL
    SELECT 'N4', '03', 'SET', 'G_CARE_LOCATION_ZIP', NULL, NULL
    FROM params p
    WHERE p.report_address = 'Y'
      AND p.service_facility_mode IN ('ALWAYS', 'CONDITIONAL')
),
...
overlay_matches AS (
    ...
    GROUP BY o.target_segment, o.field_number
),
    ...
    SELECT
        CASE
            WHEN SUM(CASE WHEN target_action = 'BLOCKED' THEN 1 ELSE 0 END) > 0
                THEN 'BLOCKED'
            WHEN SUM(CASE WHEN target_action = 'NO_CHANGE' THEN 1 ELSE 0 END) = 3
                THEN 'NO_CHANGE'
