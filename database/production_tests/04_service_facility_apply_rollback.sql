/*
 * Box 77 - Service Facility production rollback-only APPLY validation.
 *
 * This anonymous block never commits and cannot be switched to persistence.
 * It locks and re-reads the Preview 03 state, rejects a stale preview hash,
 * applies all three explicit targets atomically, verifies the temporary
...
DECLARE
    /* ================================================================
     * MANUAL INPUTS - edit only these seven constants.
     * ================================================================ */
    c_payor_guid CONSTANT VARCHAR2(36) := 'PUT_PAYOR_GUID_HERE';
    c_plan_guid CONSTANT VARCHAR2(36) := NULL;
    c_service_facility_mode CONSTANT VARCHAR2(20) := 'ALWAYS';
    ...
    BEGIN
        ...
        l_targets(1).target_segment := 'NM1';
        l_targets(1).record_type_code := 'D2310E2500NM1343';
        l_targets(2).target_order := 2;
        l_targets(2).target_segment := 'N3';
        l_targets(2).record_type_code := 'D2310E2650N3346';
        l_targets(3).target_order := 3;
        l_targets(3).target_segment := 'N4';
        l_targets(3).record_type_code := 'D2310E2700N4347';

        IF l_mode = 'ALWAYS' THEN
            l_targets(1).desired_sto_proc := 'RETURN_1';
        ELSIF l_mode = 'CONDITIONAL' THEN
            l_targets(1).desired_sto_proc := 'G_D2310E2500NM1343_COUNT';
        ELSE
            l_targets(1).desired_sto_proc := 'RETURN_0';
        END IF;
        FOR i IN 2 .. 3 LOOP
            IF l_mode = 'ALWAYS' AND l_address = 'Y' THEN
                l_targets(i).desired_sto_proc := 'RETURN_1';
            ELSIF l_mode = 'CONDITIONAL' AND l_address = 'Y' THEN
                l_targets(i).desired_sto_proc :=
                    'G_D2310E2500NM1343_COUNT';
            ELSE
                l_targets(i).desired_sto_proc := 'RETURN_0';
            END IF;
        END LOOP;
    ...
    BEGIN
        ...
        l_mode := UPPER(TRIM(c_service_facility_mode));
        l_address := UPPER(TRIM(c_report_address));
        l_expected_hash := UPPER(TRIM(c_expected_preview_state_hash));
        l_audit_user := TRIM(c_audit_user);

        IF c_safety_token <> 'ROLLBACK_ONLY_SERVICE_FACILITY' THEN
        ...
        END IF;
        IF l_mode NOT IN ('ALWAYS', 'CONDITIONAL', 'NEVER') THEN
            RAISE_APPLICATION_ERROR(c_err_invalid_input,
                'SERVICE_FACILITY_MODE is invalid.');
        END IF;
        IF l_address NOT IN ('Y', 'N') THEN
            RAISE_APPLICATION_ERROR(c_err_invalid_input,
                'REPORT_ADDRESS must be Y or N.');
        END IF;
        IF l_mode = 'NEVER' AND l_address <> 'N' THEN
            RAISE_APPLICATION_ERROR(c_err_invalid_input,
                'NEVER requires REPORT_ADDRESS N.');
        END IF;
        ...
        CASE
            WHEN l_mode = 'ALWAYS' AND l_address = 'Y' THEN
                l_option_code := 'SERVICE_FACILITY_ALWAYS_ADDRESS_YES';
            WHEN l_mode = 'ALWAYS' AND l_address = 'N' THEN
                l_option_code := 'SERVICE_FACILITY_ALWAYS_ADDRESS_NO';
            WHEN l_mode = 'CONDITIONAL' AND l_address = 'Y' THEN
                l_option_code := 'SERVICE_FACILITY_CONDITIONAL_ADDRESS_YES';
            WHEN l_mode = 'CONDITIONAL' AND l_address = 'N' THEN
                l_option_code := 'SERVICE_FACILITY_CONDITIONAL_ADDRESS_NO';
            WHEN l_mode = 'NEVER' AND l_address = 'N' THEN
                l_option_code := 'SERVICE_FACILITY_NEVER';
            ELSE
                RAISE_APPLICATION_ERROR(c_err_invalid_input,
                    'The requested mode/address pair is invalid.');
        END CASE;
        initialize_targets;
    END validate_inputs;
    ...
    BEGIN
        ...
        IF l_match_count <> 1 THEN
            RAISE_APPLICATION_ERROR(c_err_overlay,
                ...
                p_field_number || ' matched ' || l_match_count ||
    ...
    IS
        l_nm1_active     BOOLEAN;
        l_address_active BOOLEAN;
        l_index          PLS_INTEGER;
    BEGIN
        l_nm1_active := l_mode IN ('ALWAYS', 'CONDITIONAL');
        l_address_active := l_address = 'Y'
            AND l_mode IN ('ALWAYS', 'CONDITIONAL');
        ...
        IF l_targets(p_target_index).target_segment = 'NM1'
           AND l_nm1_active THEN
            set_hef_value(p_target_index, '01', 'HARD_CODED_DATA', '77');
            set_hef_value(p_target_index, '02', 'HARD_CODED_DATA', '2');
            set_hef_value(
                p_target_index, '03', 'STO_PROC_NAME', 'G_ORGANIZATION_NAME'
            );
            set_hef_value(
                p_target_index, '09', 'STO_PROC_NAME', 'G_FACILITY_NPI'
            );
        ELSIF l_targets(p_target_index).target_segment = 'N3'
              AND l_address_active THEN
            set_hef_value(
                p_target_index, '01', 'STO_PROC_NAME',
                'G_CARE_LOCATION_ADDR1'
            );
            set_hef_value(
                p_target_index, '02', 'STO_PROC_NAME',
                'G_CARE_LOCATION_ADDR2'
            );
        ELSIF l_targets(p_target_index).target_segment = 'N4'
              AND l_address_active THEN
            set_hef_value(
                p_target_index, '01', 'STO_PROC_NAME',
                'G_CARE_LOCATION_CITY'
            );
            set_hef_value(
                p_target_index, '02', 'STO_PROC_NAME',
                'G_CARE_LOCATION_STATE'
            );
            set_hef_value(
                p_target_index, '03', 'STO_PROC_NAME',
                'G_CARE_LOCATION_ZIP'
            );
        END IF;
...
BEGIN
    validate_inputs;
    SAVEPOINT service_facility_apply_test;
    ...
    DBMS_OUTPUT.PUT_LINE('==================================================');
    DBMS_OUTPUT.PUT_LINE('ROLLBACK-ONLY SERVICE FACILITY APPLY');
    ...
    IF l_recalculated_hash <> l_expected_hash THEN
        ...
        DBMS_OUTPUT.PUT_LINE(
            'STALE_PREVIEW: no DML was performed; rerun Preview 03.'
        );
    ...
    DBMS_OUTPUT.PUT_LINE('ROLLING BACK ALL SERVICE FACILITY TEST DML...');
    ROLLBACK TO service_facility_apply_test;
    l_dml_started := FALSE;
    verify_restoration;
    ROLLBACK;
    DBMS_OUTPUT.PUT_LINE(
    ...
    DBMS_OUTPUT.PUT_LINE(
        'all Service Facility DML was rolled back and original state was restored.'
    );
    DBMS_OUTPUT.PUT_LINE('==================================================');
EXCEPTION
    WHEN OTHERS THEN
        ...
        IF l_savepoint_set THEN
            BEGIN
                ...
                l_dml_started := FALSE;
                DBMS_OUTPUT.PUT_LINE(
                    'FAILURE PATH: rolled back to service_facility_apply_test.'
                );
